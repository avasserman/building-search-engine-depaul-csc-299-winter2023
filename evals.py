...
def read_queries(queries_path) -> typing.List[QueryRecord]:
    ...
    with open(queries_path) as fp:
        for line in fp:
            ...
            out.append(QueryRecord(query_id=record['_id'], query_text=record['metadata']['query']))
    return out
    ...
    with open(path) as fp:
        ...
        for line in fp:
            ...
            out.append(EvalEntry(
                query_id=record[0], doc_id=record[1], score=int(record[2])))
    return out
...
def run_eval(idx: index.Index, queries_path: str, eval_data_path: str, scorer: ResultScorer) -> float:
    ...
    for query_record in query_records:
        ...
        result_eval_entries.extend(generate_result_eval_entries(
            query_id=query_record.query_id, results=results, eval_data=eval_records))
